...
export default class AddProduct extends React.Component {
    ...
    async onChangeImage(e) {
        if (e.target.files && e.target.files[0]) {
            ...
            let fileImage = await this.convertToFile(URL.createObjectURL(img), "png", e.target.files[0].name)
            const formData = new FormData();
            formData.append("file", fileImage);
            console.log(fileImage)
            axios.post(process.env.REACT_APP_BASE_URL + "/upload_product_image", formData, {
                headers: {
    ...
    submitForm = async (e) => {
        ...
        await fetch(process.env.REACT_APP_BASE_URL + "/product/vaddproduct?c_id=" +
            ...
            this.state.qty+"&image_url="+this.state.p_image)
            .then(resp => resp.json())
            ...
            <div className='register'>
                <div className='register_container'>
                    ...
                    {this.state.p_image !== null && this.state.p_image1 == undefined &&
                        <div>
                            <img src={this.state.p_image} alt="image" style={{width:"50px",height:"50px"}} />
                        </div>
                    }
                    <Link to="/vendor"> <button className='innerbutton' type="submit" value="Submit" onClick={this.submitForm}>Add Product</button></Link><br />
